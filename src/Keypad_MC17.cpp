...
void Keypad_MC17::end( void ) {
	_wire->beginTransmission( (int)i2caddr );
	_wire->write( IOCON ); // same as when reset
	_wire->write( iocon );
	_wire->endTransmission( );
    _wire->beginTransmission( (int)i2caddr );
	_wire->write( GPPUA ); // disable pullups on all inputs
	_wire->write( 0x00 );
	_wire->write( 0x00 );
	_wire->endTransmission( );
    _wire->beginTransmission( (int)i2caddr );
	_wire->write( IODIRA ); // setup port direction - all inputs to start
	_wire->write( lowByte( 0xffff) );
	_wire->write( highByte( 0xffff ) );
	_wire->endTransmission( );
    _wire->beginTransmission( (int)i2caddr );
	_wire->write( GPIOA );	//point register pointer to gpio reg
	_wire->write( lowByte(0xffff) ); // make o/p latch agree with pulled-up pins
	_wire->write( highByte(0xffff) );
	_wire->endTransmission( );
}
